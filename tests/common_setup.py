...
def run_program(inputs):
    ...
    result = subprocess.run(
        ['python3', 'putting_python_to_work.py'],  # Adjust this command if using a different Python version or path
        input=input_data,
        text=True,
        capture_output=True,
    ...
    if check_internet_connection():
        try:
            ...
            with open('putting_python_to_work.py', 'r') as f:
                student_code = f.read()
            with open('tests/test_putting_python_to_work.py', 'r') as f:
                pytest_code = f.read()
